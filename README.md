...
```bash
git clone https://github.com/sriniously/go-taskly.git
cd go-taskly
```
...
The backend uses environment variables prefixed with `TASKLY_`. Key variables include:

- `TASKLY_DATABASE_*` - PostgreSQL connection settings
- `TASKLY_SERVER_*` - Server configuration
- `TASKLY_AUTH_*` - Authentication settings
- `TASKLY_REDIS_*` - Redis connection
- `TASKLY_EMAIL_*` - Email service configuration
- `TASKLY_OBSERVABILITY_*` - Monitoring settings

See `apps/backend/.env.example` for a complete list.

## Architecture

This taskly follows clean architecture principles:
